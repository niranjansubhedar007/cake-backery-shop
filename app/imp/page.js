...
//       const tableResponse = await axios.get(`http://localhost:5000/api/table/tables/bySection/${sectionName}/${mainTableName}`);
...
//       const sectionResponse = await axios.get(`http://localhost:5000/api/table/section/byName/${sectionName}`);
...
//       const response = await axios.put(`http://localhost:5000/api/table/${sectionId}/${mainTableId}/reset-subtables`);
...
//       const response = await axios.put(`http://localhost:5000/api/table/${sectionId}/${mainTableId}/show-subtables`, {
...
//       const itemsWithBarCategory = modifiedCurrentOrder.filter(orderItem => orderItem.barCategory);
//       const itemsWithoutBarCategory = modifiedCurrentOrder.filter(orderItem => !orderItem.barCategory);
...
//       if (orderData.itemsWithBarCategory.length === 0 && orderData.itemsWithoutBarCategory.length === 0) {
...
//       const newItemsWithBarCategory = orderData.itemsWithBarCategory.filter(newItem =>
//         !existingItems.some(existingItem => existingItem.name === newItem.name)
//       );
...
//       const uniqueItemsWithBarCategory = [...newItemsWithBarCategory, ...updatingItemsWithBarCategory];
//       const uniqueItemsWithBarCategorySet = new Set(uniqueItemsWithBarCategory.map(item => item.name));

//       const newItemsWithoutBarCategory = orderData.itemsWithoutBarCategory.filter(newItem =>
//         !existingItems.some(existingItem => existingItem.name === newItem.name)
//       );
...
//       const uniqueItemsWithoutBarCategory = [...newItemsWithoutBarCategory, ...updatingItemsWithoutBarCategory];
//       const uniqueItemsWithoutBarCategorySet = new Set(uniqueItemsWithoutBarCategory.map(item => item.name));
...
//           barCategory: orderItem.barCategory ? orderItem.barCategory : null
//         })),
//         itemsWithBarCategory: [...uniqueItemsWithBarCategorySet].map((itemName, index) => {
//           const orderItem = uniqueItemsWithBarCategory.find(item => item.name === itemName);
//           const tasteInfo = modifiedCurrentOrder.find(item => item.name === itemName);
//           return {
//             name: orderItem.name,
//             quantity: orderItem.quantity,
//             price: orderItem.price ? orderItem.price : orderItem.pricePer?.[`pricePer${orderItem.barCategory}`],
//             taste: tasteInfo && tasteInfo.selectedTaste ? tasteInfo.selectedTaste.taste : "",
//             barCategory: orderItem.barCategory ? orderItem.barCategory : null

//           };
//         }),
//         itemsWithoutBarCategory: [...uniqueItemsWithoutBarCategorySet].map((itemName, index) => {
//           const orderItem = uniqueItemsWithoutBarCategory.find(item => item.name === itemName);
//           const tasteInfo = modifiedCurrentOrder.find(item => item.name === itemName);
//           return {
//             name: orderItem.name,
//             quantity: orderItem.quantity,
...
//       if (kotData.itemsWithBarCategory.length === 0 && kotData.itemsWithoutBarCategory.length === 0) {
//         console.warn("No items in the KOT. Not saving or printing KOT.");
//         return;
//       }

//       if (kotData.itemsWithBarCategory && kotData.itemsWithBarCategory.length > 0) {
...
//         const botContent = generateOrderContent(items, tableInfo, waiterName, "BOT");
...
//         const kotContent = generateOrderContent(items, tableInfo, waiterName, "KOT");
...
//     if (items.length === 0) {
//       console.warn(`No items in the ${orderHeading}. Not generating empty preview.`);
...
//               <tr>
//                 <td>${index + 1}</td>
//                 <td class="kot-item-name">${item.name}${item.taste ? `<br>Taste - ${item.taste}` : ""}</td>
//                 <td>${item.quantity}</td>
//               </tr>
//             `).join("")}
//           </tbody>
...
//       const barCategorySubtotal = itemsWithBarCategory.reduce((total, orderItem) => {
//         const itemPrice = orderItem.price ? orderItem.price : orderItem.pricePer[`pricePer${orderItem.barCategory}`];
//         return total + (itemPrice * orderItem.quantity);
//       }, 0);

//       // Calculate subtotal for items without barCategory
//       const noBarCategorySubtotal = itemsWithoutBarCategory.reduce((total, orderItem) => {
//         const itemPrice = orderItem.price ? orderItem.price : orderItem.pricePer[`pricePer${orderItem.barCategory}`];
//         return total + (itemPrice * orderItem.quantity);
//       }, 0);
...
//         await axios.patch(`http://localhost:5000/api/kot/kot/settle/${tableId}`);
...
//       }
//       body {
//         font-family: 'DarkFont', sans-serif;
//         margin: 0;
//         padding: 0;
...
//           `<img src="http://localhost:5000/${hotelInfo.hotelLogo}" alt="Hotel Logo" style="max-width: 120px; max-height: 120px; margin-bottom: 15px;">`
//         )}
...
//         <!-- Items with barCategory Section -->
...
//                                 <td>${orderItem.quantity}</td>
//                                 <td class="totalprice">${(
//                 orderItem.price ? orderItem.price : orderItem.pricePer[`pricePer${orderItem.barCategory}`]
//               ) * orderItem.quantity.toFixed(2)}</td>
//                             </tr>`
//           )
//           .join('')}
//                 </tbody>
...
//         item.name === updatedOrderItem.name
//           ? updatedOrderItem
//           : item
//       );
...
//       const destinationTableResponse = await axios.get(`http://localhost:5000/api/table/table/bySectionAndName/${sectionId}/${destinationTableName}`);
...
//       const sourceTableResponse = await axios.get(`http://localhost:5000/api/table/table/bySectionAndName/${sectionId}/${sourceTableName}`);
//       const sourceTableId = sourceTableResponse.data._id;

//       // Call the mergeTables endpoint with the retrieved tableIds
//       const response = await axios.patch('http://localhost:5000/api/order/mergeTables', {
//         destinationTableId,
//         sourceTableId,
//       });
...
//           <div className="w-full lg:w-3/5 lg:-ml-10 bg-gray-200 -mt-3 md:w-96 relative  ">
//             {/* Header */}
...
//                       <span className="font-semibold text-xs lg:text-sm">Menu Subtotal</span>
...
//                     <div className="px-4 flex justify-between ">
//                       <span className="font-semibold text-xs lg:text-sm">Liquor Subtotal</span>
//                       <span className="font-semibold text-xs lg:text-sm">
...
//                       <div>
//                         <div className="px-4 flex justify-between ">
//                           <span className="font-bold text-xs lg:text-sm">VAT</span>
//                           <span className="font-bold">
...
//                         <span className="md:text-xs sm:text-xs lg:mb-1 flex" style={{ fontSize: '12px' }}>
//                           {product.name}
//                         </span>
//                         <span>
//                           {(product.stockQty > 0) && (
...
//             {showCategoryMenus && (
//               <div
//                 className="cursor-pointer grid grid-cols-2 bg-white md:grid-cols-3 lg:grid-cols-4 gap-3 lg:px-3 md:px-2 px-2 mt-3 custom-sidescrollbars overflow-scroll lg:max-h-[calc(67vh-1rem)] md:max-h-[calc(55vh-1rem)] max-h-[calc(97vh-1rem)] sm:max-h-[calc(80vh-1rem)]"
//               >
//                 {(menus.menus || menus)
//                   .filter(filterMenus) // Apply the filterMenus function
//                   .map((product, index) => (
...
//                         <div className="justify-center flex">
//                           <img
//                             src={product.imageUrl ? `http://localhost:5000/${product.imageUrl}` : `/wine.jpg`}
...
//             {selectedBrandMenuItem && showBrandMenus && selectedBrandMenuItem.prices.some(price => price.price > 0) && (
//               <div className="bg-white mt-3 custom-sidescrollbars overflow-scroll lg:max-h-[calc(86vh-8rem)] md:max-h-[calc(84vh-1rem)] max-h-[calc(97vh-1rem)] sm:max-h-[calc(80vh-1rem)] cursor-pointer">
//                 {/* Render prices grid here */}
//                 <div className="grid grid-cols-4 gap-4 p-4" key={selectedBrandMenuItem._id}>
//                   {/* Render child menu prices */}
//                   {selectedBrandMenuItem.prices.map((price) => (
//                     // Check if price is greater than 0 before rendering the card
//                     price.price > 0 && (
